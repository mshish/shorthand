/*
 * Throwaway visual preview harness — NOT part of the app bundle.
 *
 * Renders the settings UI's real primitives on a static page so a redesign can
 * be screenshotted without launching Tauri. Everything the settings window
 * itself renders goes through `useSettings()`, which needs Tauri IPC, so this
 * file deliberately imports only the presentational primitives from
 * `@/components/ui/` and hand-rolls the sidebar markup that `Sidebar.tsx`
 * would otherwise pull `useSettings()` in for.
 *
 * The settings rows are rendered twice, side by side: once in upstream's
 * `SettingsGroup` card ("Now") and once in the fork's borderless `Sheet`
 * ("Proposed"), so the two can be compared in one shot rather than across two.
 *
